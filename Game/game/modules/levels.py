...
def spawn_loot(loot: int, loot_rarity: str) -> List:
    """
...
level_2 = Level(name="placeholder2", entities=7, enemies=5, loot=2, loot_rarity="common")
...
def load_level(name: str | int) -> Level:
    for key in levels.keys():
        if name == key:
            level = levels[key]
            print(level)
    
    loot = spawn_loot(level.loot, level.loot_rarity)
